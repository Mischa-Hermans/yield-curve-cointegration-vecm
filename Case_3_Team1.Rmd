...
If we began by testing the level series and failed to reject the unit root null, we would still not know whether the series is truly \(I(1)\) or instead \(I(2)\). More importantly, if the series were actually \(I(2)\), an ADF test applied to the level series would not have the standard Dickey–Fuller distribution, because the first differences included in the regression would still be non-stationary. In this case, the usual critical values would be invalid, and the test result could be misleading.

The Pantula principle avoids this problem by starting from the most differenced form of the series, where stationarity is most likely. By moving step by step to lower differencing levels, unit root tests are only applied when their assumptions are satisfied. The first point at which we fail to reject the unit root therefore identifies the smallest amount of differencing required to achieve stationarity, which determines the integration order of the series.
